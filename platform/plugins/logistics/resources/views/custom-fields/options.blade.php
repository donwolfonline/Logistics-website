<script>
    window.customFields = {
        customFields: {!! Js::from($customFields) !!},
    }
</script>
...
>
        ...
        <thead>
            <tr>
                @if($isDefaultLanguage)
                    <th scope="col">#</th>
                @endif
                <th scope="col">{{ trans('plugins/logistics::logistics.custom_field.option.label') }}</th>
                <th
                    scope="col"
                    colspan="2"
                >{{ trans('plugins/logistics::logistics.custom_field.option.value') }}</th>
            </tr>
        </thead>
                    ...
                    >
                        ...
                        <td>
                            <input
                                ...
                                type="text"
                                value="{{ $value->label }}"
                                placeholder="{{ trans('plugins/logistics::logistics.custom_field.option.label') }}"
                            />
                        </td>
                        <td>
                            <input
                                ...
                                type="text"
                                value="{{ $value->value }}"
                                placeholder="{{ trans('plugins/logistics::logistics.custom_field.option.value') }}"
                            />
                        </td>
                        @if ($isDefaultLanguage)
                            <td style="width: 50px">
    ...
    @if($isDefaultLanguage)
        <button
            ...
            id="add-new-row"
            type="button"
        >{{ trans('plugins/logistics::logistics.custom_field.option.add_row') }}</button>
    @endif
</div>
